...
public class ReflectUtils {
    ...
    public static Field getField(Class<?> sourceClass, String fieldName,
        ...
        Field field = null;
        try {
            field = isFindDeclaredField ? sourceClass.getDeclaredField(fieldName) : sourceClass
                    .getField(fieldName);
            ...
            if (isUpwardFind) {
                ...
                while (field == null && classs != null) {
                    try {
                        field = isFindDeclaredField ? classs.getDeclaredField(fieldName) : classs
                                .getField(fieldName);
     ...
     * @return 给定类的所有字段
     */
    public static List<Field> getFields(Class<?> sourceClass, boolean isGetDeclaredField,
            boolean isGetParentField, boolean isGetAllParentField, boolean isDESCGet) {
        ...
        if (isGetParentField) {
            ...
            if (isDESCGet) {
                for (int w = classList.size() - 1; w > -1; w--) {
                    for (Field field : isGetDeclaredField ? classList.get(w)
                            .getDeclaredFields() : classList.get(w).getFields()) {
                        fieldList.add(field);
                    }
                }
            } else {
            ...
            for (Field field : isGetDeclaredField ? sourceClass.getDeclaredFields() : sourceClass
                    .getFields()) {
                fieldList.add(field);
            }
        }
     ...
     */
    public static boolean setField(Object object, String fieldName, Object newValue,
            boolean isFindDeclaredField, boolean isUpwardFind) {
    ...
    public static Method getMethod(Class<?> sourceClass, boolean isFindDeclaredMethod,
            boolean isUpwardFind, String methodName, Class<?>... methodParameterTypes) {
        Method method = null;
        try {
            method = isFindDeclaredMethod ? sourceClass.getDeclaredMethod(methodName,
                    methodParameterTypes) : sourceClass.getMethod(methodName, methodParameterTypes);
        } catch (NoSuchMethodException e1) {
            if (isUpwardFind) {
                Class<?> classs = sourceClass.getSuperclass();
                while (method == null && classs != null) {
                    try {
                        method = isFindDeclaredMethod ? classs.getDeclaredMethod(methodName,
                                methodParameterTypes) : classs.getMethod(methodName,
     ...
     * @return 给定类的所有方法
     */
    public static List<Method> getMethods(Class<?> clas, boolean isGetDeclaredMethod,
        ...
        if (isFromSuperClassGet) {
            ...
            if (isDESCGet) {
                for (int w = classList.size() - 1; w > -1; w--) {
                    for (Method method : isGetDeclaredMethod ? classList.get(w)
                            .getDeclaredMethods() : classList.get(w).getMethods()) {
            ...
            for (Method method : isGetDeclaredMethod ? clas.getDeclaredMethods() : clas
                    .getMethods()) {
                methodList.add(method);
            }
        }
    ...
    public static Constructor<?> getConstructor(Class<?> sourceClass,
            boolean isFindDeclaredConstructor, boolean isUpwardFind,
            Class<?>... constructorParameterTypes) {
        Constructor<?> method = null;
        try {
            method = isFindDeclaredConstructor ? sourceClass
                    .getDeclaredConstructor(constructorParameterTypes) : sourceClass
     ...
     * @return 给定的类中所有的构造函数
     */
    public static List<Constructor<?>> getConstructors(Class<?> sourceClass,
            boolean isFindDeclaredConstructor, boolean isFromSuperClassGet, boolean isDESCGet) {
        ...
        if (isFromSuperClassGet) {
            ...
            for (Constructor<?> constructor : isFindDeclaredConstructor ? sourceClass
                    .getDeclaredConstructors() : sourceClass.getConstructors()) {
                constructorList.add(constructor);
            }
        }
    ...
    public static final boolean isCollectionByType(Field field,
            Class<? extends Collection> collectionType, Class<?> type) {
        Class<?> fieldType = field.getType();
        if (collectionType.isAssignableFrom(fieldType)) {
            Class<?> first = (Class<?>) ((ParameterizedType) field.getGenericType())
